  ...
  <div class="py-4 px-4 mx-auto max-w-screen-xl lg:px-6">
      ...
      <form class="mb-8 max-w-md mx-auto">
        ...
        @endif
          ...
          <div class="relative">
              ...
              <input type="search" id="search" name="search" class="block w-full p-3 ps-9 bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand shadow-xs placeholder:text-body" placeholder="Search title posts" autocomplete="off" autofocus/>
      ...
      <div class="my-4 grid gap-8 lg:grid-cols-3 md:grid-cols-2">
        @forelse ($posts as $post)
          <article class="p-6 bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
              <div class="flex justify-between items-center mb-5 text-gray-500">
                ...
                <span class="{{ $post->category->color }} text-primary-600 text-xs font-medium inline-flex items-center px-2.5 py-0.5 rounded dark:bg-primary-200 dark:text-primary-800">
                    {{ $post->category->name }}
                </span>
                </a>
              ...
              <div class="flex justify-between items-center">
                <a href="/posts?author={{ $post->author->username }}">
                  <div class="flex items-center space-x-4">
                    <img class="w-7 h-7 rounded-full" src="{{ $post->author->avatar ? asset('storage/' . $post->author->avatar) : asset('img/default-user-avatar.png') }}" alt="{{ $post->author->name }}"/>
        ...
        @empty
          ...
          <a href="/posts" class="block mt-4 text-center text-primary-600 hover:underline">&larr; Back to all posts</a>
        @endforelse
        </div>
        {{ $posts->links() }}
      </div>
